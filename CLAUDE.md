...
- **NaN-boxed `Value` (`vm/src/value.rs`)**: `Value` is a `struct Value(u64)` (8 bytes, `Copy`) — a non-NaN `f64` *is* the number; `Nil`/`Bool`/`Symbol`(`Spur`)/`Object`(`NonNull`, via exposed-provenance) hide in the payload of a quiet NaN, tagged by a few high bits. A `ValueView` enum (`view()`) is used for exhaustive matching off the hot path; hot paths use the `as_*`/`is_*` accessors. Identifiers, string literals, and member names are interned in `Storage`'s `Rodeo` and carried as a tagged `Spur` (cheap equality; keys for globals/fields/methods, which are `FxHashMap`s). Runtime strings (concat results) are heap `LoxString` reached through an `Object` payload; the interner is permanent and never collected. The VM's `equal` op routes through `Value::as_str` so a `Symbol` and a `LoxString` with the same contents compare equal.
...
Optimizations applied (ch. 30 + two profiling-driven perf passes): `FxHashMap` for `Spur`-keyed tables, raw-byte instruction dispatch (`decode_at`), single-allocation string concat, compile-time string interning, `lto = "fat"` + `codegen-units = 1` in `[profile.release]` (the `bench` profile inherits it), a cached `*const Chunk` per `CallFrame` (kills the per-instruction closure→function→chunk downcast), a hybrid instance-field store (`SmallVec` linear-scan spilling to `FxHashMap`), `LoxClosure` as an inline-upvalue DST (one allocation), `SmallVec` for the open-upvalue/closure-capture buffers, **NaN-boxing** (`Value` is a `u64`, 8 bytes — see above), and instruction-count trims from the book's asides/challenges: in-place `Neg`/binary ops (no pop+push), a fused `OP_JMP_IF_FALSE_POP` for `if`/`while`/`for` tests (drops the separate `Pop` on both paths), and a symmetric `or` via `OP_JMP_IF_TRUE` (one conditional jump like `and`). A hand-rolled probe table was rejected as a poor fit. Per-instruction dispatch micro-opts that keep the *same* instruction count (unchecked-`get_unchecked` stack, hand-inlined arithmetic, hoisting the GC safe-point check) were each implemented and **reverted**: under fat-LTO they fell within the ±3-5% benchmark noise floor or regressed via code-layout shifts. The reliable wins are structural — cross-crate inlining, eliminating dependent loads, eliminating allocations, halving `Value`, and cutting the *number* of dispatched instructions (the book trims above).
